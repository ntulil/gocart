...
export default function CreateStore() {
            ...
            const { data } = await axios.get('/api/store/create', {headers: 
                ...
                if (['approved', 'rejected', 'pending'].includes(data.status)){
                    ...
                    switch (data.status) {
                        ...
                        case "rejected":
                            setMessage("Your store has been rejected, you can Admin for more details")
                            break;

                        case "pending":
                            setMessage("Your store request is pending, please wait for Admin while we process your request")
                            break;
    ...
    const onSubmitHandler = async (e) => {
        ...
        if (!user) {
            return toast('Please login in order to continue')
        }
        try {
            const token = await getToken()
